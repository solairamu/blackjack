...
def BlackJackGame():
    ...
    def getCardString(s, v):
        ...
        # Handle face cards and numerical cards
        valString = value_map.get(v, str(v))
        suitString = suit_map[s.lower()]
        
        # Construct the filename based on the new naming convention
        fileName = f"cards/{valString}{suitString}.svg"
        addImage(fileName)

    
    # Helper function to convert an SVG file to PNG using Inkscape
    def convert_svg_to_png(svg_path, png_path):
        # Ensure Inkscape is installed and available in your PATH.
        subprocess.run(
            ["inkscape", svg_path, "--export-type=png", "--export-filename", png_path],
            check=True
        )    

    # Modified addImage function that uses PNG files
    def addImage(fileName):
        # Replace the .svg extension with .png for the output filename.
        png_file = fileName.replace('.svg', '.png')
        
        # If the PNG doesn't exist, try converting it from the SVG.
        if not os.path.exists(png_file):
            try:
                convert_svg_to_png(fileName, png_file)
            except Exception as e:
                print(f"Error converting {fileName} to PNG: {e}")
                return

        # Now open the PNG image.
        card = Image.open(png_file)
        # Resize the card image as needed.
